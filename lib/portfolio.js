...
const RF_TOKENS = new Set(["cdb", "cra", "cri", "lca", "lf", "deb", "debenture", "tpf", "ntn-b", "ntnb", "ntn-b1", "ntnb1"]);
const INDEXER_MAP = [
  ["cdi", "CDI"],
  ["ipca", "IPCA"],
  ["pre", "Pré"],
...
function normalizeClass(value, ticker = "", name = "") {
  ...
  if (ticker) return "RV";
  for (const token of FUND_HINTS) if (s.includes(token)) return "FUNDOS";
  for (const token of RF_TOKENS) if (s.includes(token)) return "RF";
  return "OUTROS";
}
...
function parseSection(section, rowValues) {
  ...
  if (["tipo", "classe", "emissor", "taxa", "vencimento"].some((h) => slugged.includes(h))) {
    const iTipo = pick(headers, "tipo", "classe");
    const iEmissor = pick(headers, "emissor", "instituicao");
    const iTaxa = pick(headers, "taxa", "indexador");
    const iVenc = pick(headers, "vencimento");
    const iVal = pick(headers, "valor");
    const iLiq = pick(headers, "liquidez");
    const iNome = pick(headers, "nome");
    const iAcq = pick(headers, "data de aquisicao", "data aquisicao");
    ...
    return {
      ...
      indexer: normalizeIndexer(rateText, name),
      rate: pct(rateText),
  ...
  if (slugged.includes("nome")) {
    ...
    return {
      asset_class: slug(name).includes("fundo") ? "FUNDOS" : "OUTROS",
      institution: "",
      issuer: name,
      ticker: "",
      name,
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    sections.forEach((section) => {
      for (let i = section.row; i < grid.length; i += 1) {
        const parsed = parseSection(section, grid[i]);
...
export function normalizeRow(row) {
  ...
  const assetClass = normalizeClass(lower["asset class"] || lower.classe || lower.tipo || lower.asset_class || "", ticker, name);
  const issuer = normalizeInstitution(lower.emissor || lower.issuer || lower.instituicao || "");
  return {
    asset_class: assetClass,
    ...
    ticker,
    name,
    indexer: txt(lower.indexer || lower.indexador || normalizeIndexer(lower.taxa, lower.rate)),
    rate: pct(lower.taxa || lower.rate),
    maturity: parseDate(lower.vencimento || lower.maturity),
    current_value: money(lower.valor || lower["current value"] || lower.current_value || lower["valor atual"]),
    liquidity_days: parseLiquidity(lower.liquidez || lower.liquidity || lower.liquidity_days),
    acquisition_date: parseDate(lower["data de aquisicao"] || lower["data aquisicao"] || lower["acquisition date"] || lower.acquisition_date),
...
function indexerSeries(holdings) {
  const buckets = { CDI: 0, IPCA: 0, "Pré": 0, Outros: 0 };
  holdings.forEach((row) => {
    if (row.asset_class !== "RF") return;
    ...
    else if (idx.includes("PR")) buckets["Pré"] += value;
    else buckets.Outros += value;
  });
  const total = Object.values(buckets).reduce((a, b) => a + b, 0) || 1;
...
function correlationDemo(holdings) {
  const sample = holdings.slice(0, 8);
  const labels = sample.map((row) => (row.ticker || row.issuer || row.name || "").slice(0, 18));
  const matrix = sample.map((a) =>
    sample.map((b) => {
      let score = 0.08;
      if (a.asset_class === b.asset_class) score += 0.35;
      if ((a.indexer || "") === (b.indexer || "") && a.indexer) score += 0.25;
      if (slug(a.issuer) === slug(b.issuer) && a.issuer) score += 0.22;
      if (slug(a.institution) === slug(b.institution) && a.institution) score += 0.12;
      if (a.liquidity_days && b.liquidity_days && Math.abs(Number(a.liquidity_days) - Number(b.liquidity_days)) <= 5) score += 0.05;
...
function insights(holdings, byIssuer, byIndexer) {
  const total = holdings.reduce((acc, row) => acc + Number(row.current_value || 0), 0) || 1;
  const out = [];
  if (byIssuer[0] && byIssuer[0].share >= 0.2) out.push(`Alta exposição ao emissor ${byIssuer[0].label} (${(byIssuer[0].share * 100).toFixed(1)}% da carteira).`);
  if (byIndexer[0]) {
    if (byIndexer[0].label === "CDI" && byIndexer[0].share >= 0.7) out.push("Excesso em CDI pode reduzir ganho real em cenário de inflação persistente.");
    if (byIndexer[0].label === "Pré" && byIndexer[0].share >= 0.7) out.push("Carteira muito sensível a marcação a mercado por excesso de prefixados.");
  }
  const illiquid = holdings.filter((r) => (r.liquidity_days || 0) >= 30).reduce((a, b) => a + Number(b.current_value || 0), 0);
  if (illiquid / total >= 0.15) out.push("Liquidez concentrada em ativos com D+30 ou superior.");
  if (new Set(holdings.map((r) => r.asset_class)).size <= 2) out.push("Carteira conservadora com baixa diversificação entre classes.");
  if (new Set(holdings.map((r) => r.issuer).filter(Boolean)).size <= 8) out.push("Base de emissores enxuta; vale observar risco de falsa diversificação.");
...
export function computePortfolio(rows) {
  ...
  const correlation = correlationDemo(holdings);
  const score = scorePortfolio(holdings, byIssuer, byIndexer);
  const alerts = [];
  byIssuer.slice(0, 10).forEach((row) => {
    if (row.value > 250000) alerts.push({ severity: "danger", title: "FGC superado", detail: `Exposição em ${row.label} acima de R$ 250 mil.` });
    else if (row.value > 200000) alerts.push({ severity: "warning", title: "Concentração relevante", detail: `Exposição em ${row.label} acima de R$ 200 mil.` });
  });
  ...
  const illiquidShare = holdings.filter((r) => (r.liquidity_days || 0) >= 30).reduce((a, b) => a + Number(b.current_value || 0), 0) / (total || 1);
  ...
  if (byIssuer[0] && byIssuer[0].share >= 0.25) {
    alerts.push({
      severity: "warning",
      title: "Diversificação fraca",
      detail: `${byIssuer[0].label} ocupa ${(byIssuer[0].share * 100).toFixed(1)}% do patrimônio.`,
    });
  }
  ...
  return {
    ...
    correlation,
    alerts,
    insights: insights(holdings, byIssuer, byIndexer),
  };
}

export const DEMO_ROWS = [
  { asset_class: "RF", institution: "BTG", issuer: "BTG", name: "BTG CDB", indexer: "CDI", rate: 100, maturity: "2027-11-29", current_value: 27179.8, source: "demo" },
  ...
  { asset_class: "RF", institution: "Banco Arbi", issuer: "Banco Arbi", name: "Banco Arbi CDB", indexer: "CDI", rate: 114, maturity: "2031-01-20", current_value: 237121.4, source: "demo" },
