...
function parseSection(section, rowValues) {
  ...
  if (["tipo", "classe", "emissor", "taxa", "vencimento"].some((h) => slugged.includes(h))) {
    ...
    const indexer = inferRfIndexer(rateText, name, assetClass) || normalizeIndexer(rateText, name);
...
export function normalizeRow(row) {
  ...
  const indexer = inferRfIndexer(lower.taxa || lower.rate || lower.rentabilidade, name, assetClass) || normalizeIndexer(lower.taxa, lower.rate, lower.rentabilidade);
  return {
    asset_class: assetClass,
