...
function isPrefixMarker(value) {
  const s = slug(value);
  return s === "pre" || s === "prefix" || s === "prefixado";
}
...
function inferRfIndexer(rateText = "", name = "", assetClass = "", prefixMarker = "") {
  ...
  if (isPrefixMarker(prefixMarker)) return "Prefixado";
  return "";
}
...
function parseSection(section, rowValues) {
  ...
  if (["tipo", "classe", "emissor", "taxa", "vencimento"].some((h) => slugged.includes(h))) {
    ...
    const iPre = pick(headers, "pre", "prefixado", "prefix");
    ...
    const prefixMarker = iPre !== null ? vals[iPre] : vals[iTaxa + 1];
    const indexer = inferRfIndexer(rateText, name, assetClass, prefixMarker) || normalizeIndexer(rateText, name);
...
export function normalizeRow(row) {
  ...
  const indexer =
    inferRfIndexer(lower.taxa || lower.rate || lower.rentabilidade, name, assetClass, lower.pre || lower.prefixado || lower.prefix) ||
    normalizeIndexer(lower.taxa, lower.rate, lower.rentabilidade);
  return {
    asset_class: assetClass,
...
function indexerSeries(holdings) {
  const buckets = { CDI: 0, IPCA: 0, Prefixado: 0 };
  holdings.forEach((row) => {
    if (row.asset_class !== "RF") return;
    ...
    else if (idx.includes("PR")) buckets.Prefixado += value;
  });
  const total = Object.values(buckets).reduce((a, b) => a + b, 0) || 1;
