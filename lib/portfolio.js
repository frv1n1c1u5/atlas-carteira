...
function normalizeClass(value, ticker = "", name = "") {
  ...
  if (ticker) return "RV";
  if (s.includes("fii") || s.includes("fundo")) return "FUNDOS";
...
export function normalizeRow(row) {
  ...
  const rfType = inferRfType(lower.rf_type || lower.tipo_rf || lower.tipo, name);
...
export function normalizeHoldings(rows) {
  return (rows || []).map((row, idx) => {
    ...
    normalized.rf_type = normalized.asset_class === "RF" ? txt(normalized.rf_type || inferRfType(normalized.name || normalized.issuer, normalized.name)) : "";
...
function generateInsights(holdings, byIssuer, byIndexer) {
  ...
  if (byIndexer[0]) {
    ...
    if (byIndexer[0].label === "Pré" && byIndexer[0].share >= 0.7) {
